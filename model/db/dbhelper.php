...
class DBHelper
{
    ...
    public function getNewArrival($table)
    {
        ...
        $limitDate = $date->modify('-10 day');
        $newArrival = $limitDate->format('Y-m-d');
        $rows;
        $sql = "SELECT * FROM $table WHERE date_added <= ? AND date_added >= ?";
        try {
            $stmt = $this->conn->prepare($sql);
            $stmt->execute(array($startDate, $newArrival));
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOExceptio $e) {
            echo $e->getMessage();
        }
    ...
    public function updateStatusItem($table, $data, $field_id, $ref_id)
    {
        $ok;
        $sql = "UPDATE $table SET status =? WHERE $field_id = $ref_id";
        try {
            $stmt = $this->conn->prepare($sql);
            $ok = $stmt->execute($data);
        } catch (PDOException $e) {
    ...
    public function updatePrices($table, $data, $field_id, $ref_id)
    {
        $sql = "UPDATE $table SET discount_price = ?, unit_price = ?, bundled_price = ?, item_quantity = ? WHERE $field_id = $ref_id";
        try {
            $stmt = $this->conn->prepare($sql);
            $ok = $stmt->execute($data);
        } catch (PDOException $e) {
    ...
    public function updateQty($table, $data, $field_id, $ref_id)
    {
        $sql = "UPDATE $table SET item_quantity = ? WHERE $field_id = $ref_id";
        try {
            $stmt = $this->conn->prepare($sql);
            $ok = $stmt->execute($data);
        } catch (PDOException $e) {
    ...
    public function shippingDeliveryStat($table, $field_id, $ref_id)
    {
        $sql = "UPDATE $table SET delivery_status='SHIPPING' WHERE $field_id = $ref_id";
        try {
            $stmt = $this->conn->prepare($sql);
            $ok = $stmt->execute($data);
        } catch (PDOException $e) {
    ...
    public function deliveredDeliveryStat($table, $field_id, $ref_id)
    {
        $sql = "UPDATE $table SET delivery_status='DELIVERED' WHERE $field_id = $ref_id";
        try {
            $stmt = $this->conn->prepare($sql);
            $ok = $stmt->execute();
        } catch (PDOException $e) {
    ...
    public function getDeliveryNotif()
    {
        $rows;
        $sql = "SELECT * FROM delivery WHERE notif_status = 'UNREAD'";
        try {
            $stmt = $this->conn->prepare($sql);
            $stmt->execute();
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function updateDeliveryNotif($data, $field_id, $ref_id)
    {
        $ok;
        $sql = "UPDATE delivery SET notif_status = ? WHERE $field_id  = $ref_id";
        try {
            $stmt = $this->conn->prepare($sql);
            $ok = $stmt->execute($data);
        } catch (PDOException $e) {
    ...
    public function getTotalPrice($table, $field_id, $ref_id)
    {
        $rows;
        $sql = "SELECT SUM(cart.quantity * item_setup.unit_price) as total FROM cart INNER JOIN item_setup ON cart.item_id = item_setup.item_id where $field_id = $ref_id";
        try {
            $stmt = $this->conn->prepare($sql);
            $stmt->execute();
    ...
    public function getTotalPriceByItem($table, $field_id, $ref_id)
    {
        $rows;
        $sql = "SELECT cart.quantity * item_setup.unit_price as totalQty FROM cart INNER JOIN item_setup ON cart.item_id = item_setup.item_id where $field_id = $ref_id";
        try {
            $stmt = $this->conn->prepare($sql);
            $stmt->execute();
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function getSumOrders($table, $field_id, $ref_id)
    {
        $rows;
        $sql = "SELECT SUM(order_totalPrice) as total FROM orders where $field_id = $ref_id";
        try {
            $stmt = $this->conn->prepare($sql);
            $stmtm->execute();
            $rows = $stmt->fetch(PDO::FETCH_ASSOC);
